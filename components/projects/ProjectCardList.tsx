...
const projectCardClassMap = {
  ...
  active: "clicked **:[h3]:text-green-500  **:[&_p]:text-neutral-600 bg-green-200/40!"
};

export const ProjectCardList = ({ list, onSelect, selected }: Props) => {
  return (
    <ul className="sticky top-3 z-10 mb-4 flex items-center gap-4 md:mb-7">
      {list.map((name) => {
        return (
          <li key={name}>
            <button
              ...
              className={clsx(
                ...
                ctaBtnClassMap.base,
                selected == name ? ctaBtnClassMap.active : ctaBtnClassMap.inactive
              )}
            >
              {name}
            </button>
