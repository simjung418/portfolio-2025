...
import { formatComma, getRegionKey, getUniqueRegions } from "@/lib/trashbag/utils";
...
export default function TrashBagClient() {
  ...
  const { filteredList } = useShopInfo(maxDistance);
        ...
        <div>
          <ul>
            {pagedList.map((item: ShopWithD) => {
              ...
              const liKey = item.bzmnRegNo ?? `${item.stoNm}-${item.sggNm}-${item.crtrYmd}`;
              const addressType = addressTypeMap[liKey] ?? "road";
              const address = addressType === "road" ? item.lctnRoadNmAddr : item.lctnLotnoAddr;
              return (
                <li key={liKey} className="flex flex-col gap-3 border-b border-b-zinc-200 p-3">
                  <div className="flex items-baseline gap-2">
                    <h3 className="text-xl font-bold">{item.stoNm}</h3>
                        ...
                        {Object.entries(matchedPrice).map(([key, value]) => {
                          ...
                          return (
                            <li key={`${liKey}-${key}`} className="">
                              <div className="bg-zinc-100 px-3 py-1 text-center text-sm font-medium text-zinc-400">
                                {label}
                              </div>
                              <div className="px-3 py-1 text-center text-zinc-800">{value}원</div>
                            </li>
                          );
                        })}
                      </ul>
